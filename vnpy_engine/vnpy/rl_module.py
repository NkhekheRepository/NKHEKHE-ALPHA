...
class RLAgent:
    ...
    def load_checkpoint(self) -> bool:
        if self.checkpoint_file.exists():
            try:
                data = pickle.load(open(self.checkpoint_file, 'rb'))
                self.policy.set_parameters(data['parameters'])
                self.total_steps = data.get('total_steps', 0)
                logger.info(f"RL checkpoint loaded: {self.total_steps} steps")
                return True
    ...
    def save_checkpoint(self) -> bool:
        try:
            ...
            pickle.dump(data, open(self.checkpoint_file, 'wb'))
    ...
    def _state_to_observation(self, state: Dict[str, Any]) -> np.ndarray:
        ...
        for symbol in self.symbols:
            s = state.get(symbol, {})
            obs.extend([
                s.get('price', 50000) / 100.0,
                s.get('volume', 100) / 1000.0,
